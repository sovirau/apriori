...
function mining_process($db_object, $min_support, $min_confidence, $id_proses){
    //remove reset truncate (change to log mode)
    //reset_temporary($db_object);
    //get  transaksi data to array variable
    $sql_trans = mysqli_query("SELECT * FROM tb_databarang 
            WHERE status = 'aktif'");
    $dataTransaksi = $item_list = array();
    $jumlah_transaksi = mysqli_num_rows($sql_trans);
    $min_support_relative = ($min_support/$jumlah_transaksi)*100; 
    $x=0;
    while($myrow = mysqli_fetch_array($sql_trans)){
    ...
    foreach ($item_list as $key => $item) {
        $sql_jumlah = mysqli_fetch_array(mysqli_query("select nama_barang, count(*) as jumlah from tb_databarang group by nama_barang"));
        $jumlah = $sql_jumlah['jumlah'];
        $support = ($jumlah/$jumlah_transaksi) * 100;
        $lolos = ($support>=$min_support_relative)?"1":"0";
        $valueIn[] = "('$item','$jumlah','$support','$lolos','$id_proses')";
        if($lolos){
